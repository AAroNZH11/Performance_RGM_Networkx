...
ax.set_ylabel('Number of edges from new node')
...
# Add a title
ax.set_title('Execution Time of BA model')

# Add some text
text_x = len(param1)/4
text_y = len(param2)
text_z = np.max(dz)  # Adjust the position of the text along the z-axis
...
specific_ticks = np.arange(0, 0.05, 0.005)
ax.set_zticks(specific_ticks)
